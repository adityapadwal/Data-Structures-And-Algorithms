...
public:
    int solve(int index, int buy, vector<int>prices, int fee)
    {
        ...
        if(buy == 1)
        {
            ...
            int ignoreKaro = 0 + solve(index+1, 1, prices, fee);
        ...
        else
        {
            ...
            int ignoreKaro = 0 + solve(index+1, 0, prices, fee);
    ...
    int solveMem(int index, int buy, vector<int>prices, vector<vector<int>>&dp, int fee)
    {
        ...
        if(buy == 1)
        {
            ...
            int ignoreKaro = 0 + solveMem(index+1, 1, prices, dp, fee);
            profit = max(buyKaro, ignoreKaro);
        }
        else
        {
            // buy == 0
            // 2 choices => Sell & ignore/skip
            int sellKaro = prices[index] -fee + solveMem(index+1, 1, prices, dp, fee);
            int ignoreKaro = 0 + solveMem(index+1, 0, prices, dp, fee);
            profit = max(sellKaro, ignoreKaro);
        }
    ...
    int maxProfit(vector<int>& prices, int fee) {
        ...
        // Using Recursion (TLE)
        // int finalAns = solve(0, 1, prices, fee);
        // return finalAns;

        // Using Memoization (Memory Limit Exceeded)
        // int n = prices.size();
        // vector<vector<int>>dp(n, vector<int>(2, -1));
        // int finalAns = solveMem(0, 1, prices, dp, fee);
        // return finalAns;

        // Using Tabulation (Accepted)
        // int finalAns = solveTab(prices, fee);
        // return finalAns;

        // Using Space Optimization
        int finalAns = solveSo(prices, fee);
        return finalAns;
    }
};
